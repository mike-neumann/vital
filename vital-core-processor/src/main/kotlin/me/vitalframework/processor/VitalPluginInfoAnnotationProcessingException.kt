...
abstract class VitalPluginInfoAnnotationProcessingException(message: String, cause: Throwable? = null) :
    ...
    class GeneratePluginYml(cause: Throwable) : VitalPluginInfoAnnotationProcessingException(
        """
            error while generating plugin yml
            if this error persists, please open an issue on vital's github page
        """.trimIndent(),
        cause
    )

    class GeneratePluginConfigurationClass(cause: Throwable) : VitalPluginInfoAnnotationProcessingException(
        "Error while generating PluginConfiguration class",
        cause
    )
}