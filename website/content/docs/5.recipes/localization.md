...
```java
VitalLocalizationModule.Spigot.t(myPlayer, "my.resource.key.name", anArrayOfPossibleParametersForYourResourceKey);
```

Get translated text without a player:
```java
VitalLocalizationModule.Spigot.t(Locale.ENGLISH, "my.resource.key.name", anArrayOfPossibleParametersForYourResourceKey);
```

Set the language of a player:
```java
VitalLocalizationModule.Spigot.setLocale(myPlayer, Locale.ENGLISH);
```
